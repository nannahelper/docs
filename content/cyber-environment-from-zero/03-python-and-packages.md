...
## Python 项目为什么需要自己的工具箱

Python 本体是解释器，包管理器负责获取额外库，虚拟环境则把某个项目需要的解释器配置和依赖隔离开。它们不是三个互相替代的软件。把依赖全部装进系统 Python，短期看起来省事，长期容易出现一个项目升级后另一个项目突然不能运行的情况。

虚拟环境的价值不是增加步骤，而是让项目边界变得可见。进入项目时激活它，安装依赖时使用它，离开项目时可以回到系统环境；如果出现问题，也能删除并重新创建，而不会把整台电脑的 Python 搞乱。
