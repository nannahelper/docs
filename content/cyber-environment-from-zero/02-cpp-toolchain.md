...
编辑器负责让你写出源代码，编译器负责把源代码翻译成操作系统可以运行的程序。两者经常一起出现在 IDE 里，所以初学者容易以为“安装了能写代码的软件”就等于安装了完整环境。实际上，编译器版本、目标系统和命令行路径都会影响最后的结果。

我们先用很小的 C 和 C++ 程序验收工具链，是因为小程序能把问题范围缩到最小：如果连 Hello World 都无法编译，继续安装复杂框架只会增加变量。看到版本号、编译成功、程序输出正确，这三件事分别证明工具存在、转换过程完成、产物可以运行。
