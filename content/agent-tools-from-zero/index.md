...
这套学习过程本身也是迭代的：先用只读任务观察它，再开放单个工具；先让它解释错误，再让它提出修改；每次操作后检查 diff、命令输出和费用。装好 OpenCode 或其他 Agent 工具后，权限、工具和 skill 往往都可以通过对话逐步配置，但“能配置”不等于“应该全部开启”。本教程会把便利性和停止边界一起讲清楚。
