...
| 项目 | 内容 |
|:---|:---|
| **学习目标** | 掌握 DOM 操作的核心 API，理解事件驱动模型，能实现丰富的用户交互 |
| **核心比喻** | **智能家居遥控器** —— DOM 操作让你用代码"遥控"网页元素，事件让你"响应"用户的操作 |
| **预计时长** | 100 分钟 |
| **关键概念** | DOM 树、节点查询、元素增删改、事件监听、事件冒泡、事件委托、表单处理 |
| **实践任务** | 实现一个完整的 Todo 待办事项应用 |
...
## 5.7 实践任务：Todo 待办事项应用

### 任务要求

实现一个完整的 Todo 应用，包含以下功能：

- 输入框 + 添加按钮，添加新的待办事项
- 每条待办事项包含：文字内容、完成按钮、删除按钮
- 点击"完成"按钮切换完成状态（添加删除线样式）
- 点击"删除"按钮移除该条待办事项
- 使用事件委托处理完成和删除操作
- 统计并显示"未完成 / 总数"
...
2. 输入文字点击"添加"（或按 Enter），验证待办事项是否出现在列表中。
3. 点击"完成"按钮，验证文字是否添加删除线，按钮是否变为"撤销"。
