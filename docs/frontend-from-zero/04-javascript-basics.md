...
| 项目 | 内容 |
|:---|:---|
| **学习目标** | 掌握 JavaScript 核心语法，理解变量、数据类型、函数、条件与循环，建立编程思维 |
| **核心比喻** | **智能家居系统（灯光、空调、门锁）** —— JavaScript 让网页具备交互能力，就像智能家居让房子"活"起来 |
| **预计时长** | 110 分钟 |
| **关键概念** | 变量声明、数据类型、运算符、函数、条件判断、循环、数组、对象 |
| **实践任务** | 编写一个交互式计算器，能根据用户输入进行加减乘除运算 |
...
2. 输入两个数字，分别点击加减乘除按钮，验证结果是否正确。
3. 输入 0 作为第二个数字，点击除法，验证是否显示错误提示。
4. 不输入数字直接点击按钮，验证是否提示"请输入有效的数字"。
