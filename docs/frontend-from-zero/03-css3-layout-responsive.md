...
| 项目 | 内容 |
|:---|:---|
| **学习目标** | 掌握 Flexbox 和 Grid 两大现代布局系统，理解响应式设计原理，能适配手机与电脑 |
| **核心比喻** | **家具摆放（积木搭建）** —— 布局决定元素的位置关系，就像家具摆放决定房间的功能分区 |
| **预计时长** | 100 分钟 |
| **关键概念** | 文档流、Flexbox、Grid、媒体查询、响应式断点、移动优先 |
| **实践任务** | 使用 Flexbox 和 Grid 重构博客页面布局，并实现手机端适配 |
...
将第 1、2 章的博客页面改造为响应式布局：

- 手机端（< 768px）：单列布局，导航折叠为汉堡菜单
- 平板端（768px - 1024px）：两列布局（主体 + 侧边栏）
- 桌面端（≥ 1024px）：三列布局（导航 + 主体 + 侧边栏），最大宽度 1200px
...
4. 分别选择 iPhone、iPad、Desktop 三种预设，观察布局变化。
5. 手动拖拽浏览器窗口宽度，观察布局在断点处的切换效果。
